...
class DefaultDateDeserializerTest {
	@Mock
	...
	@InjectMocks
	private DefaultDateDeserializer deserializer;
	...
	void deserializeReturnsExpectedWhenTextIsValidDate() throws Exception {
		ZonedDateTime dateTime = LocalDate.of(2021, 1, 1).atStartOfDay(ZoneId.of("America/Mexico_City"));
		when(parser.getText()).thenReturn(DateTimeFormatter.ofPattern("dd/MM/yyyy", new Locale("es", "MX")).format(dateTime));
	...
	void defaultConstructorWorks() {
		DefaultDateDeserializer subject = new DefaultDateDeserializer();
