...
class VendorCreditsTest {
	...
	@InjectMocks
	private VendorCredits engine;

	@Test
	void executeCompletesSuccessfully() {
		List<Item> items = List.of(new Item(30d, 1L, 1L));
		List<Item> shrinkageItems = null;
		lenient().when(mockInventoryRepository.save(any())).then(invocation -> invocation.getArgument(0));
	...
	void executeThrowsExceptionWhenNotEnoughInventory() {
		...
		assertThrows(VendorCreditsInventoryException.class, () -> engine.execute(items, shrinkageItems, 1L));
	}
